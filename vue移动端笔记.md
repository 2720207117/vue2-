...
### 12-项目-首页-详情-评论-封装和测试api

### 13-项目-首页-详情-评论-渲染视图

### 14-项目-首页-详情-评论-获取分页数据
...
1. offset初始值为 **null**
2. ****注意last_id的用法->相当于页码 相当于时间戳
3. 文档id的来源为路由参数,这里固定为141314
...
```vue
// html    
<comment-list v-if="commentFirst" :source="commentFirst.com_id.toString()" :is-article="false" />
...
```html
            <span @click="showReply(item)">回复{{item.reply_count}}</span>

```

`注意`: 保证数据存在,然后使用

### 06-项目-首页-详情-评论-发布评论-准备组件
...
```js
      <add-comment class="add-comment" :target="article_id" :article="article" />
```
...
```html
<!--省略-->
    <add-comment class="add-comment" v-if="commetTop" :target="commetTop.com_id.toString()" :article-id="articleId" />
...
2. components/
   1. auth-info->作者信息组件
   2. more-action->点赞 不喜欢 
   3. comment-list->可复用的组件 评论的列表和回复列表
   4. add-comment->可复用的发布评论 发布回复组件
   5. reply-list.vue-> 回复组件

`思路`

1. 考虑接口是否可复用->是否可以有参数形参
2. 提供可复用组件->在写代码之前考虑的->布局slot/数据props/样式 
3. 考虑在哪用组件,考虑使用的位置是否有数据(data/computed/props/路由/vuex/ls)

### 11-项目-个人-业务分析

`提示`: 快速布局

`业务` : 需要完成头像上传(相册/拍照/对话框)
...
`明确需求`: 点击头像->打开对话框(组件)->本地相册|拍照|取消

`注意`: 这里涉及到文件的传输

### 17-项目-个人-编辑-头像上传-准备组件
